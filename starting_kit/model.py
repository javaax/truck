...
class model (BaseEstimator):
    ...
    def test():
        mod = RandomForestRegressor(max_depth=20, random_state=0,  n_estimators=100)
        
        X_random = np.random.rand(59,59)
        y_random = np.random.rand(59)
        mod.fit(X_random,y_random)
        print(mod.predict(X_random))
